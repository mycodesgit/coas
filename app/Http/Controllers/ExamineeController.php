...
class ExamineeController extends Controller
{
    ...
    public function examineeListData(Request $request)
    {
        if ($request->ajax()) {
            $examinees = Applicant::where('p_status', '=', 2)->orderBy('admission_id', 'desc')->get()
                ->map(function ($examinee) {
                    $examinee['full_name'] = ucfirst($examinee['lname']) . ', ' . ucfirst($examinee['fname']) . ' ' . ucfirst($examinee['mname']);
                    if ($examinee['mname'] != null) {
                        $examinee['full_name'] .= ' ' . ucfirst(substr($examinee['mname'], 0, 1)) . '.';
                    }
                    $examinee['type'] = $examinee['type'] == 1 ? 'New' : 'Transferee';
                    $examinee['exam_sched'] = Carbon::parse($examinee['d_admission'])->format('F d, Y') . ' - ' . Carbon::parse($examinee['time'])->format('h:i A');
                    $examinee['contact'] = '+63 ' . substr($examinee['contact'], 1);
                    return $examinee;
                });
            return response()->json(['data' => $examinees]);
        }
    ...
    public function examineeListSearchData(Request $request)
    {
        if ($request->ajax()) {
            $examinees = Applicant::where('p_status', '=', 2);
            
            if ($request->input('year')) {$examinees = $examinees->where('year', $request->input('year'));}
            if ($request->input('campus')) {$examinees = $examinees->where('campus', $request->input('campus'));}     
            if ($request->input('admission_id')) {$examinees = $examinees->where('admission_id', $request->input('admission_id'));}  
            if ($request->input('lname')) {$examinees = $examinees->where('lname', $request->input('lname'));}  
            if ($request->input('strand')) {$examinees = $examinees->where('strand', $request->input('strand'));}
        
            $examinees = $examinees->get();
            
            $examinees = $examinees->map(function ($examinee) {
                $examinee['full_name'] = ucfirst($examinee['lname']) . ', ' . ucfirst($examinee['fname']) . ' ' . ucfirst($examinee['mname']);
            ...
            $request->session()->put('recent_search', $examinees->toArray());
            $totalSearchResults = count($examinees);
        
            return response()->json(['data' => $examinees, 'totalSearchResults' => $totalSearchResults]);
        }
        
        return view('admission.examinee.srchexamineeList');
    }

...
@section('workspace')
    <ol class="breadcrumb">
        ...
        <li style="text-transform: uppercase;">{{$applicant->fname}} @if($applicant->mname == null) @else {{ substr($applicant->mname,0,1) }}.@endif {{$applicant->lname}}  @if($applicant->ext == 'N/A') @else{{$applicant->ext}}@endif</a></li>
    ...
    <form method="POST" action="{{ route('applicant_schedule_save', $applicant->id) }}" enctype="multipart/form-data">
      ...
      <div class="col-md-4">
        ...
        <select class="form-control" name="d_admission" style="text-transform: uppercase;">
          ...
          <option value="{{ $date1->date }}" @if (old('d_admission') == "{{ $date1->date }}") {{ 'selected' }} @endif>{{ $date1->date }}</option>
          @endforeach
        </select>
      </div>
      <div class="col-md-4">
        ...
        <select class="form-control" name="time" style="text-transform: uppercase;">
          ...
          <option value="{{ $time1->time }}" @if (old('time') == "{{ $time1->time }}") {{ 'selected' }} @endif>{{\Carbon\Carbon::createFromFormat('H:i:s',$time1->time)->format('h:i A')}}</option>
          @endforeach
        </select>
      </div>
      <div class="col-md-4">
        ...
        <select class="form-control" name="venue" style="text-transform: uppercase;">
          ...
          <option value="{{ $venue1->venue }}" @if (old('venue') == "{{ $venue1->venue }}") {{ 'selected' }} @endif>{{ $venue1->venue }}</option>
          @endforeach
        </select>
      </div>
      <div class="container-fluid">
